...
class WorkflowManagerTest(unittest.TestCase):
    ...
    def test_agent3_is_registered_but_not_auto_runnable(self):
        """Agent 3 V1.0 只登记交接信息，不能被 Workflow Manager 自动调度。"""
        agent = workflow_manager.AGENT_REGISTRY["agent3"]

        self.assertFalse(agent.enabled_for_cli)
        self.assertIn("短视频脚本生成", agent.description)
        self.assertIn("V1.0", agent.description)
        self.assertIn("ai-health-os-agent3-script-writer-v1.0.json", str(agent.script_path))
        self.assertIn("agent3_script_results.json", str(agent.default_output_path))

        with self.assertRaises(ValueError):
            workflow_manager.run_agent("agent3", limit=5)

    def test_agent3_n8n_workflow_is_v1_release(self):
        """Agent 3 V1.0 n8n 工作流应保持 7 个节点。"""
        workflow_path = Path(workflow_manager.AGENT_REGISTRY["agent3"].script_path)
        workflow = json.loads(workflow_path.read_text())

        self.assertEqual("AI Health OS - Agent 3 Script Writer V1.0", workflow["name"])
        self.assertEqual(7, len(workflow["nodes"]))
    ...
    def test_agent4_is_registered_but_not_auto_runnable(self):
        ...
        with self.assertRaises(ValueError):
            workflow_manager.run_agent("agent4", limit=5)
    ...
    def test_agent4_v11_is_registered_but_original_agent4_remains(self):
        """Agent 4 V1.1 作为 Agent 3 V1.1 兼容复制版登记，原 v0.1 DEV 仍保留。"""
        original = workflow_manager.AGENT_REGISTRY["agent4"]
        compatible = workflow_manager.AGENT_REGISTRY["agent4_v11"]

        self.assertIn("ai-health-os-agent4-visual-director-v0.1-dev.json", str(original.script_path))
        self.assertFalse(compatible.enabled_for_cli)
        self.assertIn("Agent 3 V1.1", compatible.description)
        self.assertIn("ai-health-os-agent4-visual-director-v1.1.json", str(compatible.script_path))
    ...
    def test_agent6_v11_is_registered_but_original_agent6_remains(self):
        """Agent 6 V1.1 作为 Agent 5 V1.1 兼容复制版登记，原 v0.1 DEV 仍保留。"""
        original = workflow_manager.AGENT_REGISTRY["agent6"]
        compatible = workflow_manager.AGENT_REGISTRY["agent6_v11"]

        self.assertIn("ai-health-os-agent6-publishing-package-analytics-v0.1-dev.json", str(original.script_path))
        self.assertFalse(compatible.enabled_for_cli)
        self.assertIn("Agent 5 V1.1", compatible.description)
        self.assertIn("ai-health-os-agent6-publishing-package-analytics-v1.1.json", str(compatible.script_path))
